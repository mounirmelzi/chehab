...
func main() {
	...
	fmt.Println("Creating bootstrapper (this may take a few minutes)...")
	btpStart := time.Now()
	bootstrapper, err = bootstrapping.NewEvaluator(btpParams, btpKeys)
	if err != nil {
		panic(err)
	}
	fmt.Printf("Bootstrapper created in %v\n", time.Since(btpStart))
	...
	// Note: x=1.0001^8192 ≈ 2.27 (stays in reasonable range)
	...
	for name, ct := range encryptedOutputs {
		...
		fmt.Printf("\nResult %s: %.6f\n", name, result[0])
		fmt.Printf("Error: %.6f (%.2f%%)\n", math.Abs(result[0]-expected), 100*math.Abs(result[0]-expected)/expected)
	}

	_ = encodedOutputs
	fmt.Println("\nCKKS computation with bootstrapping completed!")
}
