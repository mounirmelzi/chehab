...
# HEonGPU operation names for counting in generated CUDA code
heongpu_operations = ["ops.add", "ops.sub", "ops.multiply_plain", "ops.rotate_rows", "ops.negate", "ops.multiply"]
...
# Mapping: HEonGPU op name -> CSV column name
heongpu_to_csv_op = {
    "ops.add": "add",
    "ops.sub": "sub",
    "ops.multiply_plain": "multiply_plain",
    "ops.rotate_rows": "rotate_rows",
    "ops.negate": "negate",
...
output_csv = f"results_{'RL' if optimization_method == 1 else 'EGraph'}_heongpu.csv"
...
def ensure_heongpu_cmake(build_path):
    """Write a CMakeLists.txt to compile generated_fhe.cu using heongpu."""
    cmake_path = os.path.join(build_path, "CMakeLists.txt")
...
''')
    print("  [cmake] Running cmake for generated_fhe.cu...")
    subprocess.run(['cmake', '-S', '.', '-B', 'build_cu'], cwd=build_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
    subprocess.run(['cmake', '--build', 'build_cu'], cwd=build_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
...
def run_benchmark(subfolder_name, slot_count, w_ops, w_keys, build_path, build_path_he, build_path_he_build):
    ...
    for iteration in range(iterations):
        ...
        benchmark_run_command = f"./{subfolder_name} {vectorize_code} {slot_count} {optimization_method} {window_size} 1 {cse_enabled} 1 2 {w_ops} {w_keys}"
        try:
            result = subprocess.run(
        ...
        if he_build_ok:
            # ── HEonGPU: compile and run CUDA code ──────────────────
            cu_file = os.path.join(build_path, "generated_fhe.cu")
            if os.path.exists(cu_file):
                ensure_heongpu_cmake(build_path)

                if iteration == iterations - 1:
                    try:
                        for counter in range(iterations):
                            go_result = subprocess.run(
                                ['./build_cu/generated_fhe'],
                                cwd=build_path,
                                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                universal_newlines=True,
                                timeout=go_run_timeout_seconds
                            )
                            print("**HEonGPU fhe run done**")
                        ...
                        print(f"CUDA run timed out after {go_run_timeout_seconds} seconds.")
                    except Exception as e:
                        print(f"Failed running HEonGPU code for benchmark: {subfolder_name}: {e}")
            else:
                print(f"  [warn] generated_fhe.cu not found at {cu_file}")

        # Count operations from generated CUDA file
        go_gen_file = os.path.join(build_path, "generated_fhe.cu")
        if os.path.exists(go_gen_file):
            with open(go_gen_file, "r") as file:
                file_content = file.read()
                for heongpu_op, csv_op in heongpu_to_csv_op.items():
                    nb_occurrences = len(re.findall(rf'\b{heongpu_op}', file_content))
