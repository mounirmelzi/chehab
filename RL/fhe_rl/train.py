...
def train_ppo_agent(expressions_file: str, embeddings_model, total_timesteps: int = 2_000_000, num_envs: int = 8):
...
def train_lagrangian_ppo_agent(expressions_file: str, embeddings_model, total_timesteps: int = 2_000_000, num_envs: int = 8):
    benchmarks = load_expressions(str(DATA_DIR / "benchmarks.txt"))
    expressions = load_expressions(expressions_file, benchmarks)
    max_positions = 16
    rules_list  = create_rules(str(RL_DIR / "rules.txt"))
    ...
    for lagrange_iteration in range(lagrange_iterations):  # outer Lagrange loop
        ...
        model.save(f"{run_name}_iter_{lagrange_iteration}_real_data")
