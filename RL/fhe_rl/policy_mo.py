...
class HierarchicalMaskablePolicyMO(nn.Module):
    ...
    def __init__(self, observation_space: gym.Space, action_space: gym.Space,  lr_schedule: Union[float, Any], **kwargs):
        ...
        self.rule_head = mlp_mo(features_dim, rule_hidden_dims, self.rule_dim, layernorm=True, seed=seed)
        self.pos_head  = mlp_mo(features_dim + self.rule_dim, pos_hidden_dims, self.max_positions, layernorm=True, seed=seed)
        self.value_net = mlp_mo(features_dim, value_hidden_dims, 1, layernorm=True, seed=seed)
