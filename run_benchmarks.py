...
additional_infos = [
    ...
    "compile_time (s)",
    "circuit_execution_time (s)",
    "galois_keys_generation_time (s)",
        ...
        for slot_count in updated_slot_counts:
            for w in pref_list:
                ...
                try:
                    ...
                    print(
                        f"*****run {subfolder_name} , for slot_count : {slot_count}******"
                    )
                    operation_stats = {
                        "add": [],
                        "sub": [],
                    ...
                    if not subfolder_name in exceptions:
                        pro = subprocess.Popen(
                            [
                                "python3",
                                "generate_{}.py".format(subfolder_name),
                                "--slot_count",
                                str(slot_count),
                            ],
                            cwd=build_path,
                    ...
                    for iteration in range(iterations):
                        ...
                        try:
                            ...
                            for line in lines:
                                ...
                                if " ms" in line:
                                    ...
                                    operation_stats["compile_time (s)"].append(
                                        optimization_time
                                    )
                            ...
                            depth_match = re.search(
                                r"max:\s*\((\d+),\s*(\d+)\)", result.stdout
                            )
                            print(depth_match)
                            depth = int(depth_match.group(1)) if depth_match else None
                            multiplicative_depth = (
                                int(depth_match.group(2)) if depth_match else None
                            )
                            # print(depth)
                            # print(multiplicative_depth)
                            print(
                                f"Depth=>{depth}, multiplcative_depth=>{multiplicative_depth}"
                            )
                            operation_stats["Depth"].append(depth)
                            operation_stats["Multiplicative Depth"].append(
                                multiplicative_depth
                            )
                        except subprocess.TimeoutExpired:
                            print(
                                f"Command `{benchmark_run_command}` timed out after {compile_time_timeout_seconds} seconds."
                            )
                            benchmark_compilation_timed_out = True
                        except subprocess.CalledProcessError as e:
                            error_message = (
                                e.stderr if e.stderr else "No error message available."
                            )
                            print(
                                "Command for {} failed with error:\n{}".format(
                                    subfolder_name, error_message
                                )
                            )
                            continue
                        ...
                        if iteration == iterations - 1:
                            ...
                            try:
                                ...
                                for counter in range(iterations):
                                    ...
                                    if counter > 0:
                                        ...
                                        for line in lines:
                                            ...
                                            ####################################
                                            if (
                                                "galois_keys_generation_time_(ms):"
                                                in line
                                            ):
                                                # print(f"==> execution time {line.split()[0]}")
                                                execution_time = float(line.split()[1])
                                            ...
                                            if "Remaining_noise_budget:" in line:
                                                Remaining_noise_budget = int(
                                                    line.split()[1]
                                                )
                                                operation_stats[
                                                    "Remaining_noise_budget"
                                                ].append(Remaining_noise_budget)
                                            ##############
                                            if comp == 2:
                                                break
                                ...
                                print(
                                    f"Failed in building fhe_code for benchmark: {subfolder_name}"
                                )
                                continue
                        ...
                        with open(file_name, "r") as file:
                            ...
                            for op in operations:
                                nb_occurrences = len(
                                    re.findall(rf"\b{op}", file_content)
                                )
                    ...
                    if not benchmark_compilation_timed_out:
                        for key, values in operation_stats.items():
                            if values == []:
                                print(
                                    f"Warning: No values found for {key} in {subfolder_name} with slot_count {slot_count}."
                                )
                                result = "N/A"
                            else:
    ...
    for regime in regimes:
        for tree_depth in depths:
            for instance in range(
                1, number_instances_each_polynomial_configuration + 1
            ):
                for w in pref_list:
                    w_ops, w_keys = w
                    try:
                        ...
                        for iteration in range(iterations):
                            ...
                            if os.path.isdir(build_path):
                                ...
                                try:
                                    ...
                                    # print(f"\n\n {depth_match} \n\n")
                                    depth = (
                                        depth_match.group(1) if depth_match else None
                                    )
                                    multiplicative_depth = (
                                        depth_match.group(2) if depth_match else None
                                    )
                                    operation_stats["Depth"].append(int(depth))
                                    operation_stats["Multiplicative Depth"].append(
                                        int(multiplicative_depth)
                                    )
                                    print(
                                        f"Depth: {depth} --MultipliDepth {multiplicative_depth}"
                                    )
                                except subprocess.TimeoutExpired:
                                    print(
                                        f"Command `{command}` timed out after {compile_time_timeout_seconds} seconds."
                                    )
                                    benchmark_compilation_timed_out = True
                                except subprocess.CalledProcessError as e:
                                    print(
                                        f"Command for {subfolder_name} failed with error:\n{e.stderr}"
                                    )
                                if benchmark_compilation_timed_out:
                                    break
                                #########################################################################
                                ...
                                try:
                                    ...
                                    print(
                                        f"Failed in building fhe_code for benchmark:{subfolder_name} ,with error \n"
                                    )
                                build_path_he_build = os.path.join(
                                    build_path_he, "build"
                                )
                                ##########################################################################
                                if iteration == iterations - 1:
                                    try:
                                        ...
                                        print(
                                            f"Failed in running fhe_code for benchmark: {subfolder_name}"
                                        )
                                        continue
                                ###########################################################################
                            # Step 3: Parse operation counts from the generated C++ code
                        ...
                        if not benchmark_compilation_timed_out:
                            for key, values in operation_stats.items():
                                if values == []:
                                    print(
                                        f"Warning: No values found for {key} in {benchmark_name}."
                                    )
                                    result = "N/A"
                                else:
                                    result = statistics.median(values)
